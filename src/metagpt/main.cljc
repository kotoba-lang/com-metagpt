(ns metagpt.main
  "Kotodama WASM entrypoint for the metagpt clean-room actor (L4) — Clojure port.
  ...
  facts (`metagpt.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "metagpt")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"metagpt.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "metagpt-compat" :tier tier :entities entities} 200])
...
(defn start! [] :metagpt-compat/ready)
